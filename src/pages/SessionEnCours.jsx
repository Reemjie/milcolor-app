...
export default function SessionEnCours() {
  ...
  const [filterType, setFilterType] = useState(sessionStorage.getItem('session_type') || 'all')
  const [filterAnim, setFilterAnim] = useState(sessionStorage.getItem('session_anim') || 'all')
  const [search, setSearch] = useState(sessionStorage.getItem('session_search') || '')

  useEffect(() => { sessionStorage.setItem('session_type', filterType) }, [filterType])
  useEffect(() => { sessionStorage.setItem('session_anim', filterAnim) }, [filterAnim])
  useEffect(() => { sessionStorage.setItem('session_search', search) }, [search])
    ...
    <div className="page-enter" style={{ padding: '20px 16px' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, overflowX: 'auto', paddingBottom: 4 }}>
        {[
          ...
          { to: '/banque', label: "🗂 Catalogue d'animations" },
